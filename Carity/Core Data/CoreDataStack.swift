...
final class CoreDataStack: ObservableObject {
  ...
  lazy var persistentContainer: NSPersistentCloudKitContainer = {
    let container = NSPersistentCloudKitContainer(name: "CarityModel")
...
extension CoreDataStack {
  ...
  func getShare(_ patient: Patient) -> CKShare? {
    guard isShared(object: patient) else { return nil }
    guard let shareDictionary = try? persistentContainer.fetchShares(matching: [patient.objectID]),
      let share = shareDictionary[patient.objectID] else {
      print("Unable to get CKShare")
      return nil
    }
    share[CKShare.SystemFieldKey.title] = patient.fullName
    return share
  }
